/**
 * Fills the `icegate` catalog row from `config/icegateCatalog.ts` (the
 ...
 *
 *   npm run db:backfill:icegate                                      # preview
 *   npx tsx src/scripts/backfill-icegate.ts --apply                  # write
 *   npx tsx src/scripts/backfill-icegate.ts --apply --replace-copy   # write, overwriting authored copy
 *   npx tsx src/scripts/backfill-icegate.ts --apply --replace-documents
 *
 * Invoke the flagged runs through `npx tsx` as above, NOT through
 * `npm run db:backfill:icegate -- --apply`: npm consumes the flags as its own
...
import {
  ...
  ICEGATE_DOCUMENTS,
  ICEGATE_FEE_LINES,
  ICEGATE_OPTIONAL_DOCUMENT_RE,
  ICEGATE_SLUG,
  ICEGATE_WHO_CAN_APPLY,
  type IcegateFeeLine,
} from "../config/icegateCatalog.js";

const describe = (lines: IcegateFeeLine[]) =>
...
async function run() {
  ...
  const [row] = await db.select().from(services).where(eq(services.slug, ICEGATE_SLUG)).limit(1);
  if (!row) {
    console.error(`No "${ICEGATE_SLUG}" service in the catalog — run db:seed:catalog first.`);
    process.exitCode = 1;
    return;
  }
  ...
  let currentFees: IcegateFeeLine[] = [];
  try {
  ...
  const samePrice = describe(currentFees) === describe(ICEGATE_FEE_LINES);
  const writePrice = !authoredPrice || samePrice || overwritePrices;

  console.log(`  ${apply ? "updated  " : "would set"} ${ICEGATE_SLUG} (id ${row.id}, "${row.name}")`);
  console.log(
    writeCopy
  ...
  console.log(
    existingDocs.length === 0
      ? `      ${ICEGATE_DOCUMENTS.length} documents`
      : replaceDocuments
        ? `      documents REPLACED, ${existingDocs.length} removed, ${ICEGATE_DOCUMENTS.length} written`
        : `      documents kept, ${existingDocs.length} already authored`,
  );
  console.log(
    writePrice
      ? `      fee lines: ${describe(currentFees)}  →  ${describe(ICEGATE_FEE_LINES)}` +
          (authoredPrice && !samePrice ? "   ⚠ OVERWRITES the admin's price" : "")
      : `      fee lines KEPT at the admin's ${describe(currentFees)} (client price is ${describe(ICEGATE_FEE_LINES)}) — pass --overwrite-prices to reprice`,
  );
  ...
  if (apply) {
    await db
      ...
      .set({
        ...(writePrice
          ? {
              feeLines: JSON.stringify(ICEGATE_FEE_LINES),
              professionalFee: (
                ICEGATE_FEE_LINES.find((l) => /professional/i.test(l.label))?.amount ?? 0
              ).toFixed(2),
            }
          : {}),
        ...(writeCopy
          ? {
              description: ICEGATE_DESCRIPTION,
              whoCanApply: ICEGATE_WHO_CAN_APPLY,
              actsRules: ICEGATE_ACTS_RULES,
              tabs: tabsFor(ICEGATE_DESCRIPTION, ICEGATE_WHO_CAN_APPLY, ICEGATE_ACTS_RULES),
            }
          : {}),
        ...(insertDocs ? { documentsCount: ICEGATE_DOCUMENTS.length } : {}),
      })
    ...
    if (insertDocs) {
      ...
      await db.insert(documentTypes).values(
        ICEGATE_DOCUMENTS.map((name) => ({
          serviceId: row.id,
          name,
          mandatory: !ICEGATE_OPTIONAL_DOCUMENT_RE.test(name),
        })),
      );
    }
  }
...
run()
  .catch((err) => {
    console.error("ICE GATE backfill failed:", err);
    process.exitCode = 1;
  })
  .finally(async () => {
    await pool.end();
